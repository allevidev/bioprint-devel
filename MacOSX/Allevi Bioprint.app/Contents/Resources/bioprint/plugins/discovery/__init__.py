...
__plugin_url__ = "https://github.com/foosel/bioprint/wiki/Plugin:-Discovery"
__plugin_description__ = "Makes the bioprint instance discoverable via Bonjour/Avahi/Zeroconf and uPnP"
__plugin_license__ = "AGPLv3"

def __plugin_load__():
	if not pybonjour:
		# no pybonjour available, we can't use that
		logging.getLogger("bioprint.plugins." + __name__).info("pybonjour is not installed, Zeroconf Discovery won't be available")
...
class DiscoveryPlugin(bioprint.plugin.StartupPlugin,
                      bioprint.plugin.ShutdownPlugin,
	...
	@bioprint.plugin.BlueprintPlugin.route("/discovery.xml", methods=["GET"])
	def discovery(self):
		...
		if not modelName:
			...
			modelName = bioprint.server.DISPLAY_VERSION

		vendor = self._settings.get(["model", "vendor"])
		vendorUrl = self._settings.get(["model", "vendorUrl"])
		if not vendor:
			vendor = "The bioprint Project"
			vendorUrl = "http://www.bioprint.org/"
	...
	##~~ StartupPlugin API -- used for registering bioprint's Zeroconf and SSDP services upon application startup
	...
	def on_startup(self, host, port):
		...
		self.zeroconf_register("_bioprint._tcp", self.get_instance_name(), txt_record=self._create_bioprint_txt_record_dict())
	...
	##~~ ShutdownPlugin API -- used for unregistering bioprint's Zeroconf and SSDP service upon application shutdown
	...
	def zeroconf_register(self, reg_type, name=None, port=None, txt_record=None):
		...
		:param name: displayable name of the service, if not given defaults to the bioprint instance name
		:param port: port to register for the service, if not given defaults to bioprint's (public) port
	...
	def zeroconf_unregister(self, reg_type, port=None):
		...
		:param port: the port of the service to be unregistered, defaults to bioprint's (public) port if not given
		:return:
		"""
	...
	def ssdp_browse(self, query, block=True, callback=None, timeout=1, retries=5):
		...
		def browse():
			...
			for _ in xrange(retries):
				for addr in bioprint.util.interface_addresses():
					try:
	...
	def _create_http_txt_record_dict(self):
		"""
		Creates a TXT record for the _http._tcp Zeroconf service supplied by this bioprint instance.
	...
	def _create_bioprint_txt_record_dict(self):
		"""
		Creates a TXT record for the _bioprint._tcp Zeroconf service supplied by this bioprint instance.

		The following keys are defined:

		  * `path`: path prefix to actual bioprint instance, inherited from _http._tcp
		  ...
		  * `version`: bioprint software version
		  * `api`: bioprint API version
		  * `model`: Model of the device that is running bioprint
		  * `vendor`: Vendor of the device that is running bioprint
		...
		import bioprint.server
		import bioprint.server.api

		entries.update(dict(
			version=bioprint.server.VERSION,
			api=bioprint.server.api.VERSION,
			))
	...
	def _ssdp_register(self):
		"""
		Registers the bioprint instance as basic service with a presentation URL pointing to the web interface
		"""
	...
	def _ssdp_unregister(self):
		"""
		Unregisters the bioprint instance again
		"""
			...
			while (self._ssdp_monitor_active):
				try:
					...
					if not request.error_code and request.command == "M-SEARCH" and request.path == "*" and (request.headers["ST"] == "upnp:rootdevice" or request.headers["ST"] == "ssdp:all") and request.headers["MAN"] == '"ssdp:discover"':
						interface_address = bioprint.util.address_for_client(*address)
	...
	def get_instance_name(self):
		...
		if name:
			return u"bioprint instance \"{}\"".format(name)
		else:
			import socket
			return u"bioprint instance on {}".format(socket.gethostname())
