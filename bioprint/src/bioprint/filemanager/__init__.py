...
def full_extension_tree():
	...
	extension_tree_hooks = bioprint.plugin.plugin_manager().get_hooks("bioprint.filemanager.extension_tree")
	for name, hook in extension_tree_hooks.items():
		try:
			hook_result = hook()
			if hook_result is None or not isinstance(hook_result, dict):
				continue
			result = bioprint.util.dict_merge(result, hook_result)
		except:
...
class FileManager(object):
	...
	def reload_plugins(self):
		self._progress_plugins = bioprint.plugin.plugin_manager().get_implementations(bioprint.plugin.ProgressPlugin)
		self._preprocessor_hooks = bioprint.plugin.plugin_manager().get_hooks("bioprint.filemanager.preprocessor")
