# coding=utf-8
"""
This module represents bioprint's settings management. Within this module the default settings for the core
...
def settings(init=False, basedir=None, configfile=None):
    """
    Factory method for initially constructing and consecutively retrieving the :class:`~bioprint.settings.Settings`
    singleton.
    ...
    Arguments:
        init (boolean): A flag indicating whether this is the initial call to construct the singleton (True) or not
            ...
            False.
        basedir (str): Path of the base directoy for all of bioprint's settings, log files, uploads etc. If not set
            the default will be used: ``~/.bioprint`` on Linux, ``%APPDATA%/bioprint`` on Windows and
            ``~/Library/Application Support/bioprint`` on MacOS.
...
class Settings(object):
    """
    The :class:`Settings` class allows managing all of bioprint's settings. It takes care of initializing the settings
    ...
    A general word on the concept of paths, since they play an important role in bioprint's settings management. A
    ...
    def effective(self):
        ...
        return bioprint.util.dict_merge(default_settings, self._config)
    ...
    def save(self, force=False):
        ...
        from bioprint.util import atomic_write
        try:
            with atomic_write(self._configfile, "wb", prefix="bioprint-config-", suffix=".yaml") as configFile:
