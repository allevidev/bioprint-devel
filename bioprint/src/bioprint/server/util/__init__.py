...
from bioprint.users import ApiUser
...
def apiKeyRequestHandler():
	...
	if bioprint.server.appSessionManager.validate(apikey):
...
def get_user_for_apikey(apikey):
	if settings().get(["api", "enabled"]) and apikey is not None:
		if apikey == settings().get(["api", "key"]) or bioprint.server.appSessionManager.validate(apikey):
			# master key or an app session key was used
			return ApiUser()
		elif bioprint.server.userManager is not None:
			# user key might have been used
			return bioprint.server.userManager.findUser(apikey=apikey)
	return None
