	...
	def fixed_get_translations():
		...
		if translations is None:
			...
			if str(locale) != default_locale:
				# plugin translations
				plugins = bioprint.plugin.plugin_manager().enabled_plugins
...
def passive_login():
	if bioprint.server.userManager is not None:
		user = bioprint.server.userManager.login_user(flask.ext.login.current_user)
	else:
	...
	if user is not None and not user.is_anonymous():
		...
		try:
			...
			if netaddr.IPAddress(remoteAddr) in localNetworks:
				user = bioprint.server.userManager.findUser(autologinAs)
				if user is not None:
					flask.g.user = user
...
def _get_flask_user_from_request(request):
	...
	"""
	import bioprint.server.util
	import flask.ext.login
	from bioprint.settings import settings

	apikey = bioprint.server.util.get_api_key(request)
	if settings().get(["api", "enabled"]) and apikey is not None:
		user = bioprint.server.util.get_user_for_apikey(apikey)
	else:
		user = flask.ext.login.current_user
...
def restricted_access(func):
	...
	done for bioprint's Access Control plus that any conditions of the
	...
	If bioprint's Access Control has not been setup yet (indicated by the "firstRun"
	...
	def decorated_view(*args, **kwargs):
		...
		apikey = bioprint.server.util.get_api_key(flask.request)
		if apikey == bioprint.server.UI_API_KEY:
			# UI API key => call regular login_required decorator, we are using browser sessions here
			
			#if socket key is same as bioprint.server.SOCKET_KEY allow
			socketkey = bioprint.server.util.get_socket_key(flask.request)	#if socket key matches allow access
			if socketkey == bioprint.server.SOCKET_KEY:
				return func(*args, **kwargs)
...
def collect_plugin_assets(enable_gcodeviewer=True, preferred_stylesheet="css"):
	...
	if preferred_stylesheet == "less":
		assets["less"].append('less/bioprint.less')
	elif preferred_stylesheet == "css":
		assets["css"].append('css/bioprint.css')
