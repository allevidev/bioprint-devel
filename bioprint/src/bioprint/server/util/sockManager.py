...
class SocketManager(sockjs.tornado.SockJSConnection, bioprint.printer.PrinterCallback, Thread):
	...
	def __init__(self, printer, fileManager, analysisQueue, userManager, eventManager, pluginManager, session):
		...
		self.cloudSocketManagerURL = os.environ.get('CLOUD_SOCKET_MANAGER_URL')	# Biobots cloud socket manager URL
		self.cloudSocketManagerPort = os.environ.get('CLOUD_SOCKET_MANAGER_PORT')	#	 Biobots cloud socket manager PORT
		self.printerId = os.environ.get('PRINTER_ID') #	This Printer's ID
		...
		self.bioprintBaseUrl = 'http://localhost:8090/api/'  #	This will always be localhost (bioprint url)
	...
	def _on_command_cloud(self, *args):
		...
		if (args[0]['type'] == 'GET'):
			r = requests.get(
				...
				headers={
					'x-api-key': bioprint.server.UI_API_KEY,
	...
	def _emit(self, type, payload):
		if not self.connected:
			...
			self._logger.warn("Unable to emit data. No client connected to printer")	 
		else:
			try:
				self.cloudManagerSocket.emit('SEND_MESSAGE', {
		      'targetClientType': 'USER',
		      'targetClient': 'rahul.fakir@biobots.io',
		      'command': 'UPDATE',
		      'response': { 
		      	'type': payload
		     	},
				...
				self._logger.warn("Could not send update data to client") 
