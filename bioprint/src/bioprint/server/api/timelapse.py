...
def getTimelapseData():
	timelapse = bioprint.timelapse.current

	config = {"type": "off"}
	if timelapse is not None and isinstance(timelapse, bioprint.timelapse.ZTimelapse):
		config["type"] = "zchange"
		config["postRoll"] = timelapse.post_roll
		config["fps"] = timelapse.fps
	elif timelapse is not None and isinstance(timelapse, bioprint.timelapse.TimedTimelapse):
		config["type"] = "timed"
		config["postRoll"] = timelapse.post_roll
		config["fps"] = timelapse.fps
		config.update({
	...
	files = bioprint.timelapse.getFinishedTimelapses()
	for file in files:
...
def setTimelapseConfig():
	if "type" in request.values:
		...
		if admin_permission.can() and "save" in request.values and request.values["save"] in valid_boolean_trues:
			bioprint.timelapse.configureTimelapse(config, True)
		else:
			bioprint.timelapse.configureTimelapse(config)
