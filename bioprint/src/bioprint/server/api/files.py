...
from bioprint.server.util.flask import restricted_access, get_json_command_from_request
...
def _getFileList(origin, filter=None):
	if origin == FileDestinations.SDCARD:
		...
		filter_func = None
		if filter:
			filter_func = lambda entry, entry_data: bioprint.filemanager.valid_file_type(entry, type=filter)
		files = fileManager.list_files(origin, filter=filter_func, recursive=False)[origin].values()
		for file in files:
			file["origin"] = FileDestinations.LOCAL

			if "analysis" in file and bioprint.filemanager.valid_file_type(file["name"], type="gcode"):
				file["gcodeAnalysis"] = file["analysis"]
				del file["analysis"]

			if "history" in file and bioprint.filemanager.valid_file_type(file["name"], type="gcode"):
				# convert print log
...
def uploadGcodeFile(target):
	...
	if input_upload_name in request.values and input_upload_path in request.values:
		upload = bioprint.filemanager.util.DiskFileWrapper(request.values[input_upload_name], request.values[input_upload_path])
	else:
	...
	def fileProcessingFinished(filename, absFilename, destination):
		...
		if destination == FileDestinations.SDCARD and bioprint.filemanager.valid_file_type(filename, "gcode"):
	...
	def selectAndOrPrint(filename, absFilename, destination):
		...
		"""
		if bioprint.filemanager.valid_file_type(added_file, "gcode") and (selectAfterUpload or printAfterSelect or (currentFilename == filename and currentOrigin == destination)):
	...
	if bioprint.filemanager.valid_file_type(added_file, "stl"):
		filename = added_file
		done = True
	else:
...
def gcodeFileCommand(filename, target):
	...
	if command == "select":
		...
		except bioprint.slicing.UnknownSlicer as e:
			return make_response("Slicer {slicer} is not available".format(slicer=e.slicer), 400)

		if not bioprint.filemanager.valid_file_type(filename, type="stl"):
		...
		if slicer_instance.get_slicer_properties()["same_device"] and (printer.is_printing() or printer.is_paused()):
			# slicer runs on same device as bioprint, slicing while printing is hence disabled
		...
		except bioprint.slicing.UnknownProfile:
...
class WerkzeugFileWrapper(bioprint.filemanager.util.AbstractFileWrapper):
	"""
	...
	def __init__(self, file_obj):
		bioprint.filemanager.util.AbstractFileWrapper.__init__(self, file_obj.filename)
