...
from bioprint.server import app, userManager, pluginManager, gettext, \
...
def index():
	...
	hooks = pluginManager.get_hooks("bioprint.ui.web.templatetypes")
	...
	templates["settings"]["entries"] = dict(
		...
		section_bioprint=(gettext("bioprint"), None),
	...
	if locale != "en":
		...
		for name, plugin in plugins.items():
			...
			for dirname in dirs:
				...
				if plugin_messages is not None:
					messages = bioprint.util.dict_merge(messages, plugin_messages)
		...
		for dirname in dirs:
			...
			if core_messages is not None:
				messages = bioprint.util.dict_merge(messages, core_messages)
