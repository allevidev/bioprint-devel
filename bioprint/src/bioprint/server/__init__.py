...
# only import the bioprint stuff down here, as it might depend on things defined above to be initialized already
from bioprint.printer import get_connection_options
from bioprint.printer.profile import PrinterProfileManager
from bioprint.printer.standard import Printer
from bioprint.settings import settings
import bioprint.users as users
import bioprint.events as events
import bioprint.plugin
import bioprint.timelapse
import bioprint._version
import bioprint.util
import bioprint.filemanager.storage
import bioprint.filemanager.analysis
...
class Server():
	...
	def run(self):
		...
		analysisQueue = bioprint.filemanager.analysis.AnalysisQueue()
		slicingManager = bioprint.slicing.SlicingManager(s.getBaseFolder("slicingProfiles"), printerProfileManager)
		storage_managers = dict()
		storage_managers[bioprint.filemanager.FileDestinations.LOCAL] = bioprint.filemanager.storage.LocalFileStorage(s.getBaseFolder("uploads"))
		fileManager = bioprint.filemanager.FileManager(analysisQueue, slicingManager, printerProfileManager, initial_storage_managers=storage_managers)
		printer = Printer(fileManager, analysisQueue, printerProfileManager)
		appSessionManager = util.flask.AppSessionManager()
		pluginLifecycleManager = LifecycleManager(pluginManager)

		def bioprint_plugin_inject_factory(name, implementation):
			if not isinstance(implementation, bioprint.plugin.bioprintPlugin):
				return None
			return dict(
		...
		def settings_plugin_inject_factory(name, implementation):
			if not isinstance(implementation, bioprint.plugin.SettingsPlugin):
				return None
			...
			plugin_settings = bioprint.plugin.plugin_settings(name,
		...
		pluginManager.implementation_inject_factories=[bioprint_plugin_inject_factory, settings_plugin_inject_factory]
		pluginManager.initialize_implementations()

		settingsPlugins = pluginManager.get_implementations(bioprint.plugin.SettingsPlugin)
		...
		def template_enabled(name, plugin):
			if plugin.implementation is None or not isinstance(plugin.implementation, bioprint.plugin.TemplatePlugin):
				return
		...
		# configure timelapse
		bioprint.timelapse.configureTimelapse()
		...
		if s.getBoolean(["accessControl", "enabled"]):
			...
			try:
				clazz = bioprint.util.get_class(userManagerName)
				userManager = clazz()
		...
		def mime_type_guesser(path):
			from bioprint.filemanager import get_mime_type
		...
		for name, hook in pluginManager.get_hooks("bioprint.server.http.routes").items():
			try:
		...
		server_routes.append((r".*", util.tornado.UploadStorageFallbackHandler, dict(fallback=util.tornado.WsgiInputContainer(app.wsgi_app), file_prefix="bioprint-file-upload-", file_suffix=".tmp", suffixes=upload_suffixes)))
		...
		for name, hook in pluginManager.get_hooks("bioprint.server.http.bodysize").items():
			try:
		...
		bioprint.plugin.call_plugin(bioprint.plugin.StartupPlugin,
		                             "on_startup",
		                             args=(self._host, self._port))

		def call_on_startup(name, plugin):
			implementation = plugin.get_implementation(bioprint.plugin.StartupPlugin)
		...
		def on_shutdown():
			...
			observer.stop()
			observer.join()
			bioprint.plugin.call_plugin(bioprint.plugin.ShutdownPlugin,
			                             "on_shutdown")
		...
		try:
			# this is the main loop - as long as tornado is running, bioprint is running
			ioloop.start()
		except (KeyboardInterrupt, SystemExit):
			pass
		except:
			self._logger.fatal("Now that is embarrassing... Something really really went wrong here. Please report this including the stacktrace below in bioprint's bugtracker. Thanks!")
	...
	def _check_for_root(self):
		if "geteuid" in dir(os) and os.geteuid() == 0:
			exit("You should not run bioprint as root!")
	...
	def _get_locale(self):
		...
		if hasattr(g, "identity") and g.identity and userManager is not None:
			...
			except bioprint.users.UnknownUser:
				pass
	...
	def _setup_logging(self, debug, logConf=None):
		...
		config = bioprint.util.dict_merge(defaultConfig, configFromFile)
	...
	def _register_template_plugins(self):
		template_plugins = pluginManager.get_implementations(bioprint.plugin.TemplatePlugin)
	...
	def _setup_blueprints(self):
		...
		from bioprint.server.apps import apps, clear_registered_app
		import bioprint.server.views
	...
	def _register_blueprint_plugins(self):
		blueprint_plugins = bioprint.plugin.plugin_manager().get_implementations(bioprint.plugin.BlueprintPlugin)
	...
	def _register_asset_plugins(self):
		asset_plugins = bioprint.plugin.plugin_manager().get_implementations(bioprint.plugin.AssetPlugin)
		for plugin in asset_plugins:
			if isinstance(plugin, bioprint.plugin.BlueprintPlugin):
				continue
			try:
	...
	def _register_blueprint_plugin(self, plugin):
		...
		if plugin.is_blueprint_protected():
			from bioprint.server.util import apiKeyRequestHandler, corsResponseHandler
