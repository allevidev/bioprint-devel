...
class CuraPlugin(bioprint.plugin.SlicerPlugin,
	...
	def __init__(self):
		self._logger = logging.getLogger("bioprint.plugins.cura")
		self._cura_logger = logging.getLogger("bioprint.plugins.cura.engine")
	...
	@bioprint.plugin.BlueprintPlugin.route("/import", methods=["POST"])
	def import_cura_profile(self):
		import datetime
		import tempfile

		from bioprint.server import slicingManager
		...
		profile_description = "Imported from {filename} on {date}".format(filename=filename, date=bioprint.util.get_formatted_datetime(datetime.datetime.now()))
		...
		if "allowOverwrite" in flask.request.values:
			from bioprint.server.api import valid_boolean_trues
		...
		except bioprint.slicing.ProfileAlreadyExists:
	...
	def get_slicer_profile(self, path):
		...
		return bioprint.slicing.SlicingProfile(properties["type"], "unknown", profile_dict, display_name=display_name, description=description)

	def save_slicer_profile(self, path, profile, allow_overwrite=True, overrides=None):
		if os.path.exists(path) and not allow_overwrite:
			raise bioprint.slicing.ProfileAlreadyExists("cura", profile.name)
	...
	def do_slice(self, model_path, printer_profile, machinecode_path=None, profile_path=None, position=None, on_progress=None, on_progress_args=None, on_progress_kwargs=None):
		try:
			...
			try:
				...
				while p.returncode is None:
					...
					line = bioprint.util.to_unicode(line, errors="replace")
			...
			with self._job_mutex:
				if machinecode_path in self._cancelled_jobs:
					...
					raise bioprint.slicing.SlicingCancelled()
		...
		except bioprint.slicing.SlicingCancelled as e:
			raise e
		except:
	...
	def _save_profile(self, path, profile, allow_overwrite=True):
		import yaml
		with bioprint.util.atomic_write(path, "wb") as f:
...
__plugin_url__ = "https://github.com/foosel/bioprint/wiki/Plugin:-Cura"
__plugin_description__ = "Adds support for slicing via CuraEngine from within bioprint"
