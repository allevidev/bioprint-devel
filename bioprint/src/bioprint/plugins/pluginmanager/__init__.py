...
from bioprint.settings import valid_boolean_trues
from bioprint.server.util.flask import restricted_access
from bioprint.server import admin_permission
from bioprint.util.pip import PipCaller, UnknownPip
...
class PluginManagerPlugin(bioprint.plugin.SimpleApiPlugin,
                          bioprint.plugin.TemplatePlugin,
                          bioprint.plugin.AssetPlugin,
                          bioprint.plugin.SettingsPlugin,
                          bioprint.plugin.StartupPlugin,
                          bioprint.plugin.BlueprintPlugin):
	...
	def initialize(self):
		self._console_logger = logging.getLogger("bioprint.plugins.pluginmanager.console")
	...
	@bioprint.plugin.BlueprintPlugin.route("/upload_archive", methods=["POST"])
	@restricted_access
	...
	def on_api_get(self, request):
		...
		return jsonify(plugins=result,
		               ...
		               os=self._get_os(),
		               bioprint=self._get_bioprint_version(),
		               pip=dict(
	...
	def command_install(self, url=None, path=None, force=False, reinstall=None, dependency_links=False):
		...
		#
		#   Successfully installed bioprint-Plugin-1.0 Dependency-One-0.1 Dependency-Two-9.3
		#
		# or this:
		#
		#   Successfully installed bioprint-Plugin Dependency-One Dependency-Two
		#   Cleaning up...
		#
		...
		for key, plugin in all_plugins_after.items():
			...
			self._logger.warn("The plugin was installed successfully, but couldn't be found afterwards to initialize properly during runtime. Please restart bioprint.")
		...
		needs_refresh = new_plugin.implementation and isinstance(new_plugin.implementation, bioprint.plugin.ReloadNeedingPlugin)
	...
	def command_uninstall(self, plugin):
		...
		needs_refresh = plugin.implementation and isinstance(plugin.implementation, bioprint.plugin.ReloadNeedingPlugin)
		...
		if not needs_restart:
			...
			except bioprint.plugin.core.PluginLifecycleException as e:
	...
	def command_toggle(self, plugin, command):
		...
		except bioprint.plugin.core.PluginNeedsRestart:
	...
	def _fetch_repository_from_url(self):
		...
		try:
			import json
			with bioprint.util.atomic_write(self._repository_cache_path, "wb") as f:
	...
	def _refresh_repository(self, repo_data=None):
		...
		bioprint_version = self._get_bioprint_version()
		if "-" in bioprint_version:
			bioprint_version = bioprint_version[:bioprint_version.find("-")]
		...
		def map_repository_entry(entry):
			...
			if "compatibility" in entry:
				if "bioprint" in entry["compatibility"] and entry["compatibility"]["bioprint"] is not None and len(entry["compatibility"]["bioprint"]):
					result["is_compatible"]["bioprint"] = self._is_bioprint_compatible(bioprint_version, entry["compatibility"]["bioprint"])
	...
	def _is_bioprint_compatible(self, bioprint_version_string, compatibility_entries):
		"""
		Tests if the current ``bioprint_version`` is compatible to any of the provided ``compatibility_entries``.
		"""

		bioprint_version = pkg_resources.parse_version(bioprint_version_string)
		for octo_compat in compatibility_entries:
			if not any(octo_compat.startswith(c) for c in ("<", "<=", "!=", "==", ">=", ">", "~=", "===")):
				octo_compat = ">={}".format(octo_compat)

			s = next(pkg_resources.parse_requirements("bioprint" + octo_compat))
			if bioprint_version in s:
				break
		else:
			return False
	...
	def _get_bioprint_version(self):
		from bioprint._version import get_versions
...
__plugin_url__ = "https://github.com/foosel/bioprint/wiki/Plugin:-Plugin-Manager"
__plugin_description__ = "Allows installing and managing bioprint plugins"
...
def __plugin_load__():
	...
	__plugin_hooks__ = {
		"bioprint.server.http.bodysize": __plugin_implementation__.increase_upload_bodysize
	}
