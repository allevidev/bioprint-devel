...
def test_simple_periodic_send():
    ...
    msg = can.Message(arbitration_id=0x000, data=[0, 0, 0, 0, 0, 0])
    task = can.send_periodic('can0', msg, 0.20)
    time.sleep(2)
    task.stop()
...
def test_periodic_send_with_modifying_data():
    ...
    msg = can.Message(arbitration_id=0x000, data=[1, 1, 1, 1])
    task = can.send_periodic('can0', msg, 0.20)
    time.sleep(2)
...
def test_dual_rate_periodic_send():
    ...
    msg = can.Message(arbitration_id=0x000, data=[0, 1, 2, 3, 4, 5])
    print("Creating cyclic task to send message 10 times at 1ms, then every 500ms")
    task = can.interface.MultiRateCyclicSendTask('can0', msg, 10, 0.001, 0.50)
    time.sleep(2)
