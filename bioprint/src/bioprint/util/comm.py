...
class CANCom(object):
    ...
    def sendCommand(self, cmd, cmd_type=None, processed=False):
        ...
        if self.isPrinting() or len(self._sendingLocks) > 0:
    ...
    def _readline(self):
        ...
        try:
            ...
            if not self._connection_closing:
                self._log("Unexpected error while reading from CAN bus")
    ...
    def _monitor(self):
        ...
        while self._monitoring_active:
            try:
                ...
                self._logger.exception("Something crashed inside the serial connection loop, please report this in bioprint's bug tracker:")
    ...
    def _readline(self):
        ...
        try:
            ...
            if not self._connection_closing:
                ...
                self._log("Unexpected error while reading serial port, please consult bioprint.log for details: %s" % (get_exception_string()))
                self._errorValue = get_exception_string()
                self.close(True)
            return None
        if ret == '':
