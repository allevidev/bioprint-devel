# coding=utf-8
"""
In this module the slicing support of bioprint is encapsulated.
...
class TemporaryProfile(object):
	...
	Arguments:
	    save_profile (callable): Method to use for saving the temporary profile, also responsible for applying the
	        ...
	        :meth:`~bioprint.plugin.SlicerPlugin.save_slicer_profile`.
...
class SlicingManager(object):
	...
	Arguments:
	    ...
	    printer_profile_manager (~bioprint.printer.profile.PrinterProfileManager): :class:`~bioprint.printer.profile.PrinterProfileManager`
	...
	def initialize(self):
		...
		:class:`~bioprint.plugin.SlicerPlugin` implementations.
		"""
		self.reload_slicers()

	def reload_slicers(self):
		"""
		Retrieves all registered :class:`~bioprint.plugin.SlicerPlugin` implementations and registers them as
		available slicers.
		"""
		plugins = bioprint.plugin.plugin_manager().get_implementations(bioprint.plugin.SlicerPlugin)
		slicers = dict()
	...
	def get_slicer(self, slicer, require_configured=True):
		...
		Raises:
		    ~bioprint.slicing.exceptions.UnknownSlicer: The ``slicer`` is unknown.
		    ~bioprint.slicing.exceptions.SlicerNotConfigured: The ``slicer`` is not yet configured and ``require_configured`` was set to True.
		"""
	...
	def slice(self, slicer_name, source_path, dest_path, profile_name, callback,
		...
		Raises:
		    ~bioprint.slicing.exceptions.UnknownSlicer: The slicer specified via ``slicer_name`` is unknown.
		    ~bioprint.slicing.exceptions.SlicerNotConfigured: The slice specified via ``slicer_name`` is not configured yet.
		"""
	...
	def load_profile(self, slicer, name, require_configured=True):
		...
		Raises:
		    ~bioprint.slicing.exceptions.UnknownSlicer: The slicer specified via ``slicer`` is unknown.
		    ~bioprint.slicing.exceptions.SlicerNotConfigured: The slicer specified via ``slicer`` has not yet been configured and
		        ``require_configured`` was True.
		    ~bioprint.slicing.exceptions.UnknownProfile: The profile for slicer ``slicer`` named ``name`` does not exist.
		"""
	...
	def save_profile(self, slicer, name, profile, overrides=None, allow_overwrite=True, display_name=None, description=None):
		...
		Raises:
		    ...
		    ~bioprint.slicing.exceptions.UnknownSlicer: The slicer ``slicer`` is unknown.
		    ~bioprint.slicing.exceptions.ProfileAlreadyExists: A profile with name ``name`` already exists for ``slicer`` and ``allow_overwrite`` is
		        False.
		"""
		if not slicer in self.registered_slicers:
			raise UnknownSlicer(slicer)
	...
	def all_profiles(self, slicer, require_configured=False):
		...
		Arguments:
		    ...
		    require_configured (boolean): Whether to require the slicer ``slicer`` to be already configured (True)
		        or not (False, default). If False and the slicer is not yet configured, a :class:`~bioprint.slicing.exceptions.SlicerNotConfigured`
		...
		Raises:
		    ~bioprint.slicing.exceptions.UnknownSlicer: The slicer ``slicer`` is unknown.
		    ~bioprint.slicing.exceptions.SlicerNotConfigured: The slicer ``slicer`` is not configured and ``require_configured`` was True.
		"""
	...
	def get_profile_path(self, slicer, name, must_exist=False):
		...
		Raises:
		    ~bioprint.slicing.exceptions.UnknownSlicer: The slicer ``slicer`` is unknown.
		    ~bioprint.slicing.exceptions.UnknownProfile: The profile named ``name`` doesn't exist and ``must_exist`` was True.
		"""
