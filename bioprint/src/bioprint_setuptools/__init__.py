...
class ExtractTranslation(Command):
	...
	mail_address = "i18n@bioprint.org"
	copyright_holder = "The bioprint Project"
	mapping_file = None
	pot_file = None
	input_dirs = None

	@classmethod
	def for_options(cls, mail_address="i18n@bioprint.org", copyright_holder="The bioprint Project", mapping_file=None, pot_file=None, input_dirs=None):
...
class RefreshTranslation(Command):
	...
	@classmethod
	def for_options(cls, mail_address="i18n@bioprint.org", copyright_holder="The bioprint Project", mapping_file=None, pot_file=None, input_dirs=None, output_dir=None):
		if mapping_file is None:
			raise ValueError("mapping_file must not be None")
		if pot_file is None:
			raise ValueError("pot_file must not be None")
...
def create_plugin_setup_parameters(identifier="todo", name="TODO", version="0.1", description="TODO", author="TODO",
	...
	if package is None:
		package = "bioprint_{identifier}".format(**locals())
	...
	if "bioprint" not in requires:
		requires = ["bioprint"] + list(requires)
	...
	return dict(
		...
		# Hook the plugin into the "bioprint.plugin" entry point, mapping the plugin_identifier to the plugin_package.
		# That way bioprint will be able to find the plugin and load it.
		entry_points={
			"bioprint.plugin": ["{identifier} = {package}".format(**locals())]
		}
	)
