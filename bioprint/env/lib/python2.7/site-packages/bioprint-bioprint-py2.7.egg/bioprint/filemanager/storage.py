...
class StorageInterface(object):
	"""
	Interface of storage adapters for bioprint.
	"""
	...
	def analysis_backlog(self):
		"""
		Get an iterator over all items stored in the storage that need to be analysed by the :class:`~bioprint.filemanager.AnalysisQueue`.
...
class LocalFileStorage(StorageInterface):
	...
	def _analysis_backlog_generator(self, path=None):
		...
		for entry in os.listdir(path):
			if entry.startswith(".") or not bioprint.filemanager.valid_file_type(entry):
				continue
	...
	def add_file(self, path, file_object, printer_profile=None, links=None, allow_overwrite=False):
		path, name = self.sanitize(path)
		if not bioprint.filemanager.valid_file_type(name):
	...
	def set_additional_metadata(self, path, key, data, overwrite=False, merge=False):
		...
		if not key in metadata[name] or overwrite:
			...
			new_data = bioprint.util.dict_merge(current_data, data)
	...
	def _add_links(self, name, path, links):
		file_type = bioprint.filemanager.get_file_type(name)
		if file_type:
	...
	def _list_folder(self, path, filter=None, recursive=True):
		...
		for entry in os.listdir(path):
			...
			if os.path.isfile(entry_path):
				file_type = bioprint.filemanager.get_file_type(entry)
				if not file_type:
