...
class VirtualPrinter():
	...
	def __init__(self, read_timeout=5.0, write_timeout=10.0):
		import logging
		self._logger = logging.getLogger("bioprint.plugin.virtual_printer.VirtualPrinter")
		...
		self._action_hooks = plugin_manager().get_hooks("bioprint.plugin.virtual_printer.custom_action")
	...
	def _processIncoming(self):
		...
		while self.incoming is not None and not self._killed:
			...
			if data.strip() == "version":
				...
				self.outgoing.put("bioprint VirtualPrinter v" + get_versions()["version"])
				continue
