# coding=utf-8
"""
This module represents bioprint's plugin subsystem. This includes management and helper methods as well as the
...
from bioprint.plugin.core import (PluginInfo, PluginManager, Plugin)
from bioprint.plugin.types import *

from bioprint.util import deprecated
...
def _validate_plugin(phase, plugin_info):
	if phase == "after_load":
		if plugin_info.implementation is not None and isinstance(plugin_info.implementation, AppPlugin):
			...
			import warnings
			warnings.warn("{name} uses deprecated plugin mixin AppPlugin, use bioprint.accesscontrol.appkey hook instead".format(name=plugin_info.key), DeprecationWarning)

			hooks = plugin_info.hooks
			if not "bioprint.accesscontrol.appkey" in hooks:
				hooks["bioprint.accesscontrol.appkey"] = plugin_info.implementation.get_additional_apps
...
def plugin_manager(init=False, plugin_folders=None, plugin_types=None, plugin_entry_points=None, plugin_disabled_list=None,
	...
	"""
	Factory method for initially constructing and consecutively retrieving the :class:`~bioprint.plugin.core.PluginManager`
	singleton.
	...
	Returns:
	    PluginManager: A fully initialized :class:`~bioprint.plugin.core.PluginManager` instance to be used for plugin
	        management tasks.
	...
	if _instance is not None:
		...
		if init:
			...
			if plugin_entry_points is None:
				plugin_entry_points = "bioprint.plugin"
			...
			_instance = PluginManager(plugin_folders,
			                          plugin_types,
			                          plugin_entry_points,
			                          logging_prefix="bioprint.plugins.",
...
def call_plugin(types, method, args=None, kwargs=None, callback=None, error_callback=None):
	...
	.. sourcecode:: python
	   ...
	   bioprint.plugin.call_plugin(
	       [bioprint.plugin.StartupPlugin],
	       "on_startup",
...
class PluginSettings(object):
	...
	:class:`bioprint.settings.Settings` interfaces as well as means to access plugin specific folder locations.
	...
	Arguments:
	    settings (Settings): The :class:`~bioprint.settings.Settings` instance on which to operate.
	...
	def global_get(self, path, **kwargs):
		...
		Directly forwards to :func:`bioprint.settings.Settings.get`.
		"""
		return self.settings.get(path, **kwargs)

	def global_get_int(self, path, **kwargs):
		"""
		Like :func:`global_get` but directly forwards to :func:`bioprint.settings.Settings.getInt`.
		"""
		return self.settings.getInt(path, **kwargs)

	def global_get_float(self, path, **kwargs):
		"""
		Like :func:`global_get` but directly forwards to :func:`bioprint.settings.Settings.getFloat`.
		"""
		return self.settings.getFloat(path, **kwargs)

	def global_get_boolean(self, path, **kwargs):
		"""
		Like :func:`global_get` but directly orwards to :func:`bioprint.settings.Settings.getBoolean`.
		"""
	...
	def global_set(self, path, value, **kwargs):
		...
		Directly forwards to :func:`bioprint.settings.Settings.set`.
		"""
		self.settings.set(path, value, **kwargs)

	def global_set_int(self, path, value, **kwargs):
		"""
		Like :func:`global_set` but directly forwards to :func:`bioprint.settings.Settings.setInt`.
		"""
		self.settings.setInt(path, value, **kwargs)

	def global_set_float(self, path, value, **kwargs):
		"""
		Like :func:`global_set` but directly forwards to :func:`bioprint.settings.Settings.setFloat`.
		"""
		self.settings.setFloat(path, value, **kwargs)

	def global_set_boolean(self, path, value, **kwargs):
		"""
		Like :func:`global_set` but directly forwards to :func:`bioprint.settings.Settings.setBoolean`.
		"""
	...
	def global_get_basefolder(self, folder_type, **kwargs):
		...
		:func:`bioprint.settings.Settings.getBaseFolder`.
		"""
	...
	def get_plugin_logfile_path(self, postfix=None):
		...
		Plugins may use this for specific logging tasks. For example, a :class:`~bioprint.plugin.SlicingPlugin` might
	...
	@deprecated("PluginSettings.get_plugin_data_folder has been replaced by bioprintPlugin.get_plugin_data_folder",
	            includedoc="Replaced by :func:`~bioprint.plugin.types.bioprintPlugin.get_plugin_data_folder`",
	            since="1.2.0")
