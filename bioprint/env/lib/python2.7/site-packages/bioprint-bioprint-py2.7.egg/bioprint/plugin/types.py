# coding=utf-8
"""
This module bundles all of bioprint's supported plugin implementation types as well as their common parent
class, :class:`bioprintPlugin`.
...
class bioprintPlugin(Plugin):
	"""
	The parent class of all bioprint plugin mixins.

	.. attribute:: _plugin_manager

	   The :class:`~bioprint.plugin.core.PluginManager` instance. Injected by the plugin core system upon
	   initialization of the implementation.

	.. attribute:: _printer_profile_manager

	   The :class:`~bioprint.printer.profile.PrinterProfileManager` instance. Injected by the plugin core system upon
	   initialization of the implementation.

	.. attribute:: _event_bus

	   The :class:`~bioprint.events.EventManager` instance. Injected by the plugin core system upon initialization of
	   the implementation.

	.. attribute:: _analysis_queue

	   The :class:`~bioprint.filemanager.analysis.AnalysisQueue` instance. Injected by the plugin core system upon
	   initialization of the implementation.

	.. attribute:: _slicing_manager

	   The :class:`~bioprint.slicing.SlicingManager` instance. Injected by the plugin core system upon initialization
	   of the implementation.

	.. attribute:: _file_manager

	   The :class:`~bioprint.filemanager.FileManager` instance. Injected by the plugin core system upon initialization
	   of the implementation.

	.. attribute:: _printer

	   The :class:`~bioprint.printer.PrinterInterface` instance. Injected by the plugin core system upon initialization
	   of the implementation.

	.. attribute:: _app_session_manager

	   The :class:`~bioprint.users.SessionManager` instance. Injected by the plugin core system upon initialization of
	   the implementation.

	.. attribute:: _plugin_lifecycle_manager

	   The :class:`~bioprint.server.LifecycleManager` instance. Injected by the plugin core system upon initialization
	   of the implementation.
...
class StartupPlugin(bioprintPlugin):
	"""
	The ``StartupPlugin`` allows hooking into the startup of bioprint. It can be used to start up additional services
...
class ShutdownPlugin(bioprintPlugin):
	"""
	The ``ShutdownPlugin`` allows hooking into the shutdown of bioprint. It's usually used in conjunction with the
	...
	def on_shutdown(self):
		"""
		Called upon the imminent shutdown of bioprint.
		"""
		pass


class AssetPlugin(bioprintPlugin, RestartNeedingPlugin):
	"""
	...
	def get_assets(self):
		...
		The assets will be made available by bioprint under the URL ``/plugin/<plugin identifier>/static/<path>``, with
		``plugin identifier`` being the plugin's identifier and ``path`` being the path as defined in the asset dictionary.

		Assets of the types ``js``, ``css`` and ``less`` will be automatically bundled by bioprint using
...
class TemplatePlugin(bioprintPlugin, ReloadNeedingPlugin):
	"""
	Using the ``TemplatePlugin`` mixin plugins may inject their own components into the bioprint web interface.

	Currently bioprint supports the following types of injections out of the box:
	...
	the "helloworld" plugin in bioprint's :ref:`Plugin Tutorial <sec-plugins-gettingstarted>`.
	...
	Plugins can also add additional template types by implementing the :ref:`bioprint.ui.web.templatetypes <sec-plugins-hook-ui-web-templatetypes>` hook.
	"""

	def get_template_configs(self):
		"""
		Allows configuration of injected navbar, sidebar, tab and settings templates (and also additional templates of
		types specified by plugins through the :ref:`bioprint.ui.web.templatetypes <sec-plugins-hook-ui-web-templatetypes>` hook).
...
class SimpleApiPlugin(bioprintPlugin):
	"""
	...
	The simple API constructed by bioprint for you will be made available under ``/api/plugin/<plugin identifier>/``.
	bioprint will do some preliminary request validation for your defined commands, making sure the request body is in
	...
	.. code-block:: python
	   ...
	   class MySimpleApiPlugin(bioprint.plugin.SimpleApiPlugin):
	...
	and print something like this line to ``bioprint.log``::

	   2015-02-12 17:40:21,140 - bioprint.plugins.mysimpleapiplugin - INFO - command2 called, some_parameter is some_value
	...
	def on_api_command(self, command, data):
		"""
		Called by bioprint upon a POST request to ``/api/plugin/<plugin identifier>``. ``command`` will contain one of
		...
		If your plugin returns nothing here, bioprint will return an empty response with return code ``204 No content``
		...
		:return: ``None`` in which case bioprint will generate a ``204 No content`` response with empty body, or optionally
	...
	def on_api_get(self, request):
		"""
		Called by bioprint upon a GET request to ``/api/plugin/<plugin identifier>``. ``request`` will contain the
...
class BlueprintPlugin(bioprintPlugin, RestartNeedingPlugin):
	"""
	...
	.. code-block:: python
	   ...
	   class MyBlueprintPlugin(bioprint.plugin.BlueprintPlugin):
	       @bioprint.plugin.BlueprintPlugin.route("/echo", methods=["GET"])
	       def myEcho(self):
	...
	Your blueprint will be published by bioprint under the base URL ``/plugin/<plugin identifier>/``, so the above
...
class SettingsPlugin(bioprintPlugin):
	"""
	Including the ``SettingsPlugin`` mixin allows plugins to store and retrieve their own settings within bioprint's
	configuration.
	...
	.. code-block:: python
	   ...
	   class MySettingsPlugin(bioprint.plugin.SettingsPlugin, bioprint.plugin.StartupPlugin):
	       ...
	       def on_settings_save(self, data):
	           ...
	           bioprint.plugin.SettingsPlugin.on_settings_save(self, data)
	...
	.. attribute:: _settings
	   ...
	   The :class:`~bioprint.plugin.PluginSettings` instance to use for accessing the plugin's settings. Injected by
	...
	def on_settings_save(self, data):
		...
		additional settings properties that are not stored within bioprint's configuration.
		...
		merged = bioprint.util.dict_merge(current, data)
	...
	def get_settings_version(self):
		...
		Use this to have bioprint trigger your migration function if it detects an outdated settings version in
		config.yaml.
	...
	def on_settings_migrate(self, target, current):
		"""
		Called by bioprint if it detects that the installed version of the plugin necessitates a higher settings version
		...
		Your plugin's implementation should take care of migrating any data by utilizing self._settings. bioprint
...
class EventHandlerPlugin(bioprintPlugin):
	"""
	The ``EventHandlerPlugin`` mixin allows bioprint plugins to react to any of :ref:`bioprint's events <sec-events>`.
	bioprint will call the :func:`on_event` method for any event fired on its internal event bus, supplying the
	event type and the associated payload. Please note that until your plugin returns from that method, further event
	processing within bioprint will block - the event queue itself is run asynchronously from the rest of bioprint,
	...
	def on_event(self, event, payload):
		"""
		Called by bioprint upon processing of a fired event on the platform.
...
class SlicerPlugin(bioprintPlugin):
	"""
	Via the ``SlicerPlugin`` mixin plugins can add support for slicing engines to be used by bioprint.

	"""

	def is_slicer_configured(self):
		"""
		Unless the return value of this method is ``True``, bioprint will not register the slicer within the slicing
	...
	def get_slicer_properties(self):
		...
		same_device
		    True if the slicer runs on the same device as bioprint, False otherwise. Slicers running on the same
		...
		progress_report
		    ``True`` if the slicer can report back slicing progress to bioprint ``False`` otherwise.
	...
	def get_slicer_default_profile(self):
		"""
		Should return a :class:`~bioprint.slicing.SlicingProfile` containing the default slicing profile to use with
		this slicer if no other profile has been selected.

		Returns:
		    SlicingProfile: The :class:`~bioprint.slicing.SlicingProfile` containing the default slicing profile for
		        this slicer.
		"""
		return None
	...
	def get_slicer_profile(self, path):
		"""
		Should return a :class:`~bioprint.slicing.SlicingProfile` parsed from the slicing profile stored at the
		indicated ``path``.
	...
	def save_slicer_profile(self, path, profile, allow_overwrite=True, overrides=None):
		"""
		Should save the provided :class:`~bioprint.slicing.SlicingProfile` to the indicated ``path``, after applying
	...
	def do_slice(self, model_path, printer_profile, machinecode_path=None, profile_path=None, position=None, on_progress=None, on_progress_args=None, on_progress_kwargs=None):
		"""
		Called by bioprint to slice ``model_path`` for the indicated ``printer_profile``. If the ``machinecode_path`` is ``None``,
		slicer implementations should generate it from the provided ``model_path``.

		If provided, the ``profile_path`` is guaranteed by bioprint to be a serialized slicing profile created through the slicing
		...
		Please note that both ``on_progress_args`` and ``on_progress_kwargs`` as supplied by bioprint might be ``None``,
		...
		_analysis
		    ...
		    :class:`~bioprint.filemanager.analysis.GcodeAnalysisQueue` for GCODE files.
		...
		If the job gets cancelled, a :class:`~bioprint.slicing.SlicingCancelled` exception should be raised.
...
class ProgressPlugin(bioprintPlugin):
	"""
	...
	def on_print_progress(self, storage, path, progress):
		"""
		Called by bioprint on minimally 1% increments during a running print job.
	...
	def on_slicing_progress(self, slicer, source_location, source_path, destination_location, destination_path, progress):
		"""
		Called by bioprint on minimally 1% increments during a running slicing job.
...
class AppPlugin(bioprintPlugin):
	"""
	...
	.. deprecated:: 1.2.0
	   ...
	   Refer to the :ref:`bioprint.accesscontrol.appkey hook <sec-plugins-hook-accesscontrol-appkey>` instead.
